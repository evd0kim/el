...
import { HTLCERC20ABI } from './htlc-erc20-abi'
...
//ABI
const { abi: htlcContractABI } = HTLCERC20ABI
const contractABI = ERC20ABI
let cachedContract: ethers.Contract | null

export const getContract = async (contractAddress: string) => {
  if (!cachedContract) {
    cachedContract = await getContractInstance(contractAddress, HTLCERC20ABI.abi)
  }
...
export const newContract = async (
  ...
  console.log('amountWbtc', hashLock)
