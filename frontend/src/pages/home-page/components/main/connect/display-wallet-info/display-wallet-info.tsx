...
import IconAlby from '~/shared/icons/IconAlby'
...
export const DisplayWalletInfo = ({ onClick, variant }: Props) => {
  const albyWallet = useAppSelector(state => state.alby)
  const metaMaskWallet = useAppSelector(state => state.metaMask)
  const { connect, nameAlby, nameMetaMask } = connectData.info
  ...
  const renderWalletIcon = (variant: Variant) => {
    switch (variant) {
      case nameAlby:
        return <IconAlby className={s.DisplayWalletInfoSucceededIcon} />
      case nameMetaMask:
